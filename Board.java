...
public class Board {
   ...
   */
  private void win(Character c, int[] coords){
  ...
  private void setPosition(int[] newCoords, int[] location){
    ...
    if (this.board.get(newCoords[1]).get(newCoords[0]) instanceof Carrot){
      ...
      if (this.debug) System.out.println(this.c.getNameChar() + " found " + carrot.toString());

      this.c.setCarrot(carrot);
    }

    else if (this.c.getNameChar() == "marvin" && this.board.get(newCoords[1]).get(newCoords[0])
          instanceof Character){
      Character deadChar = Character.class.cast(this.board.get(newCoords[1]).get(newCoords[0]));
      if (deadChar.hasCarrot()) this.c.setCarrot(deadChar.kill());
      else deadChar.kill();
    }

    if (this.debug) System.out.println("Moving the Character from " + Arrays.toString(location) + " to " + Arrays.toString(newCoords) + "\n");
  ...
  public boolean moveCharacter(Character c){
    ...
    if (this.debug){
      ...
      for (int x = 0; x < this.characters.length; x++){
        System.out.println(this.characters[x].getNameChar() + " is at postion " + Arrays.toString(this.characters[x].getLocation()));
      }

      for (int x = 0; x < 2; x++){
        System.out.println(this.carrotArr[x].toString() + " is at postion " + Arrays.toString(this.carrotArr[x].getLocation()));
      }
    ...
    if (this.debug) System.out.println("The valid moves are: " + validMoves.toString());

    if (this.c.isWinner()) return true;

    int[] location = this.c.getLocation();
    if (this.debug) this.c.toStringL();

    Collections.shuffle(validMoves);

    if (this.debug) System.out.println("The case we want is " + validMoves.get(0));

    switch(validMoves.get(0)){

      case 1:

        if (this.debug) System.out.println("Top Left...");
      ...
      case 2:
        if (this.debug) System.out.println("Top...");
      ...
      case 3:
        ...
        if (this.debug) System.out.println("Top Right...");
      ...
      case 4:
        ...
        if (this.debug) System.out.println("Middle Left...");
      ...
      case 6:
        ...
        if (this.debug) System.out.println("Middle Right...");
      ...
      case 7:
        ...
        if (this.debug) System.out.println("Bottom Left...");
      ...
      case 8:
        ...
        if (this.debug) System.out.println("Bottom...");
      ...
      case 9:
        ...
        if (this.debug) System.out.println("Bottom Right...");
   ...
   *
   * @param  this.c Character player being tested
   * @return   ArrayList<Integer> list of valid moves
   */
  public ArrayList<Integer> testMoves(){

    if (this.debug) System.out.println("Testing for Moves...");

    ArrayList<Integer> moves = new ArrayList();
    int[] location = this.c.getLocation();

    if (this.c.getNameChar() == "marvin"){
      if (this.debug) System.out.println("We have Marvin looking for a move...");
      for (int i = -1, k = 1; i<2; i++){
        for (int j = -1; j < 2; j++, k++){
          ...
          int x = location[0]+j;
          int y = location[1]+i;
          if (this.debug) System.out.println("x: "+x+" y: "+y);


          try {
            if (debug) System.out.println("[" + x + ", " + y + "] is " + this.board.get(y).get(x));

            if (this.c.hasCarrot() && this.board.get(y).get(x) instanceof Mountain){
              if (this.debug) System.out.println("Marvin is climbing the mountain...");
              int[] coords = {x,y};
              win(this.c, coords);
              break;
            }
            ...
            else if (this.board.get(y).get(x) == " " ||this.board.get(y).get(x) instanceof Carrot || this.board.get(y).get(x) instanceof Character
                    && !(Character.class.cast(this.board.get(y).get(x)).getNameChar().equals(this.c.getNameChar()))){

              if (this.debug) System.out.println("Adding " + k + " to the list because [" + x + ", " + y + "] is " + this.board.get(y).get(x).toString());
      ...
      for (int i = -1, k = 1; i<2; i++){
        for (int j = -1; j < 2; j++, k++){
          ...
          try {
            ...
            if (this.c.hasCarrot() && this.board.get(y).get(x) instanceof Mountain){
              if (this.debug) System.out.println(this.c.toString()+" is climbing the mountain...");
              int[] coords = {x,y};
              win(this.c, coords);
              break;
            }
              ...
              if (this.debug) System.out.println("Adding " + k + " to the list because [" + x + ", " + y + "] is " + this.board.get(y).get(x));
  ...
  public void moveMountain(){
    ...
    if (this.debug) System.out.println("Moving Mountains...");

    ArrayList<int[]> openPositions = new ArrayList();
    for (int i = 0; i < this.board.size(); i++){
      for (int j = 0; j < this.board.get(i).size(); j++){
        if (this.board.get(i).get(j) == " ") openPositions.add(new int[] {j,i});
      }
    }
    ...
    System.out.println("Moving mountain from " + Arrays.toString(this.mountain.getLocation()) + " to " + Arrays.toString(openPositions.get(0)));
  ...
  public String printBoard(){
    ...
    if (this.debug) System.out.println("Printing Board...");
  ...
  private void createBoard(){
    ...
    if (this.debug) System.out.println(Arrays.toString(deck));
    ...
    for(i = 0; i < this.characters.length; i++){
      ...
      int row = (deck[i] / 5);
      int column = (deck[i] % 5);
      if (this.debug) System.out.println("row: " + row + " column " + column + " deck[i] " + deck[i]);
    ...
    if (this.debug){
        for (int j = 0; j < this.characters.length; j++){
          System.out.println(this.characters[j].getNameChar() + " is at postion " + Arrays.toString(this.characters[j].getLocation()));
        }
    }
    ...
    for (int j = 0; j < 2; j++){
      System.out.println(this.carrotArr[j].toString() + " is at postion " + Arrays.toString(this.carrotArr[j].getLocation()));
    }
    ...
    if (this.debug) System.out.println(this.mountain.toString() + " is at postition " + Arrays.toString(this.mountain.getLocation()));
